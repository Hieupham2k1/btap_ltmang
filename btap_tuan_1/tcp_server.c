...
while (1)
{
    ...
    FILE *greetingFilePointer;
    char greetingString[100];
    greetingFilePointer = fopen("read.txt", "r");

    fgets(greetingString, 100, greetingFilePointer);
    fclose(greetingFilePointer);

    // Gửi dữ liệu sang client
    send(client, greetingString, strlen(greetingString), 0);
